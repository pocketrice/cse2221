...
public final class NaturalNumberStaticOps {
    ...
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        if (n.compareTo(ONE_THOUSAND) >= 0) {
            NaturalNumber ndiv = new NaturalNumber1L(n);
            ndiv.divide(ONE_THOUSAND);
            printWithCommas(ndiv, out);

            String nStr = n.toString();
            out.print("," + nStr.substring(
                    nStr.length() - (ONE_THOUSAND.toString().length() - 1)));
        } else {
            out.print(n);
        }
    }
    ...
    private static String toStringWithCommas(NaturalNumber n) {
        ...
        if (n.compareTo(ONE_THOUSAND) >= 0) {
            ...
            output.append(toStringWithCommas(ndiv) + "," + nStr.substring(
                    nStr.length() - (ONE_THOUSAND.toString().length() - 1)));
        } else {
            output.insert(0, nStr);
        }
