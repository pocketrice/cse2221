...
public final class PizzaOrderManager {
    ...
    public static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            priceMap.add(pair[0], Integer.parseInt(pair[1]));
        }
        in.close();
    }
     ...
     * </pre>
     */
    public static int getOneOrder(SimpleReader input,
        ...
        String orderLine;
        int price = 0;
        price += sizePriceMap.value(input.nextLine());

        do {
            orderLine = input.nextLine();

            if (!orderLine.isBlank()) {
                price += toppingPriceMap.value(orderLine);
            }
        } while (!orderLine.isBlank());
     ...
     * </pre>
     */
    public static void putPrice(SimpleWriter output, int price) {
        ...
        int cents = price % 100;
        String paddedCents = String.valueOf(cents);

        if (cents < 10) {
            paddedCents = "0" + paddedCents;
        }

        output.println("$" + dollars + "." + paddedCents);
    }
